...
class MovementHistory(models.Model):
    current_ward_number = models.CharField(verbose_name='Текущий номер палаты', max_length=3, unique=True, validators=[
        RegexValidator(regex='^[0-9]{3}$', message='Упс... Попробуйте снова')], help_text='В формате - 010')
    next_ward_number = models.CharField(verbose_name='Номер будущей палаты', max_length=3, unique=True, validators=[
        RegexValidator(regex='^[0-9]{3}$', message='Упс... Попробуйте снова')], help_text='В формате - 010')
    patient = models.ForeignKey('Patient', verbose_name='Пациент', on_delete=models.PROTECT)
    ward_movement_date = models.DateTimeField(verbose_name='Дата перемещения', auto_now=True)
...
class Patient(models.Model):
    ...
    change_ward_date = models.DateField(verbose_name='Дата назнечения в палату')
